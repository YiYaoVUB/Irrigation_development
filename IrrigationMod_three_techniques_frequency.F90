...
contains
  ...
  subroutine CalcIrrigationNeeded(this, bounds, num_exposedvegp, filter_exposedvegp, &
    ...
    !
    ! !LOCAL VARIABLES:
    integer :: fp, p    ! patch index
    integer :: fc, c    ! column index
    integer :: g    ! gridcell index
    integer :: j    ! level
       ...
       if (check_for_irrig_patch(p)) then
          ...
          if(this%irrig_method_patch(p) == irrig_method_drip  .or.  this%irrig_method_patch(p) == irrig_method_sprinkler) then
             ...
             this%irrig_rate_demand_patch(p) = deficit(c) / &
                  (this%dtime*this%irrig_nsteps_per_day)
             !write(iulog,*) 'irrig_method_patch(p) = ', irrig_method_patch(p)
             !write(iulog,*) 'sfc_irrig_rate_patch(p) = ', sfc_irrig_rate_patch(p)
          else if(this%irrig_method_patch(p) == irrig_method_flood) then
  ...
  function PointNeedsCheckForIrrig(this, pft_type, elai, londeg) &
    ...
    if (pftcon%irrigated(pft_type) == 1._r8 .and. &
       ...
       if (seconds_since_irrig_start_time < this%dtime .and. mod(days_of_year, 3) == 0) then
